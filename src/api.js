...
const checkToken = async (accessToken) => {
    const result = await fetch(
        ` https://qjro534x8h.execute-api.us-west-2.amazonaws.com/dev/api/get-events/{access_token}`
    )
        .then((res) => res.json())
...
export const getAccessToken = async () => {
    ...
    if (!accessToken || tokenCheck.error) {
        ...
        if (!code) {
            const results = await axios.get('https://qjro534x8h.execute-api.us-west-2.amazonaws.com/dev/api/get-auth-url');
...
const getToken = async (code) => {
    ...
    const { access_token } = await fetch(' https://qjro534x8h.execute-api.us-west-2.amazonaws.com/dev/api/token' + '/' + encodeCode)
        .then((res) => {
            return res.json();
        })
        .catch((error) => error);
